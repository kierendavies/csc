...
Click File->Import...
Select "Existing Project into Workspace" (under "General") and click next
...
Select 5.0 or higher
  ...
  Note: Make sure that you have JDK 1.6 installed! Click on the link at the bottom saying Configure the Installed JRE's and see if Java 1.5 or 1.6 is there. If not you need to Add it (for TSL computers select the 1.6 JDK folder in /usr/lib/jvm/)
...
For "Destination Folder:" choose the *same* directory (must be the same otherwise the library files won't be copied to the new folder)
Click Finish
You should see the "SurvivalGame" project on the left now.
Right click on it and click "Properties"
Click on the "Run" tab on the left
In the textfield next to "VM Options" enter in -Djava.library.path="libs/jogl-1.1.1-linux-i586/lib/" (or -Djava.library.path="libs/jogl-1.1.1-windows-i586/lib/" or -Djava.library.path="libs/jogl-1.1.1-windows-amd64/lib/" depending on your OS)
Click "OK" and you are ready to build and run

Post on the forums if you have a problem. Please be sure to specify exactly what is wrong and provide any relevant information that I might need.
